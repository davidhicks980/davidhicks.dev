...
export class HicksResumeEntry extends LitElement {
  ...
  watchScroll() {
    ...
    this.controllers.intersection
      ...
      .on(IntersectionObserverFilter.INTERSECTING)
      .subscribe((entries: IntersectionObserverEntry[]) => {
        const id = (entries[0]?.target as HicksResumeEntry)?.entryId;
        if (this.entryId === id) {
          this.isActive = true;
        }
      });
  ...
  private expandIcon = svg`<svg class="expand" xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 0 24 24" width="24px" fill="currentColor"><path d="M0 0h24v24H0V0z" fill="none"/> 
