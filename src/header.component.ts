import { LitElement, html, css } from 'lit';
import { customElement, query } from 'lit/decorators.js';
import anime from 'animejs';
import { fromEvent, BehaviorSubject, Subject } from 'rxjs';
import {
  debounceTime,
  distinctUntilChanged,
  map,
  tap,
  throttleTime,
} from 'rxjs/operators';
import { StateController } from './component-state';
import { style } from './header.css';
@customElement('background-animation')
export class BackgroundAnimation extends LitElement {
  @query('.header__grid')
  container: HTMLElement;
  @query('.header__upper')
  upper: HTMLElement;
  @query('.header__title__icon-wrapper')
  titleEl: HTMLHeadingElement;
  @query('.header__title-wrapper')
  titleWrapper: HTMLDivElement;
  @query('.header__title__icon')
  paths: SVGPathElement;
  @query('.header__toolbar')
  toolbar: HTMLDivElement;
  @query('.polymorph')
  polymorph: SVGPathElement;
  stateObserver: StateController;
  scale = new BehaviorSubject(1);
  constructor() {
    super();
  }
  connectedCallback() {
    super.connectedCallback();
    this.stateObserver = new StateController(this);
  }
  updateState(toolbarHeight: number) {
    if (this.stateObserver)
      this.stateObserver.updateState({
        toolbarHeight: `${toolbarHeight}vh`,
      });
  }

  getTranslation() {
    const getY = () => {
      let topToLogoMidpoint =
        this.titleWrapper.offsetTop + this.titleWrapper.offsetHeight / 2;
      let midLogoToUpperBottom = this.upper.offsetHeight - topToLogoMidpoint;

      let upperBottomToToolbarMid =
        this.polymorph.getBoundingClientRect().height / 2;
      return midLogoToUpperBottom + upperBottomToToolbarMid;
    };
    const translateX = this.titleWrapper.offsetLeft * -0.5;

    return { translateY: getY(), translateX };
  }
  getTranslateAnimation(translate: { translateY: number; translateX: number }) {
    return {
      targets: this.titleEl,
      ...translate,
      easing: 'linear',
      autoplay: false,
    };
  }
  firstUpdated(_changedProperties) {
    let toolbarTranslate = parseInt(
      getComputedStyle(this)
        .getPropertyValue('--toolbar-height')
        .replace('vh', '')
    );
    this.updateState(toolbarTranslate);
    let translateSubject = new Subject();
    const resizeObserver = new ResizeObserver((entries) => {
      for (let entry of entries) {
        translateSubject.next(entry);
      }
    });

    //let translateY = (toolbarTranslate + heightTranslate) / 2.05 / scale + 'vh';
    //   let translateX = this.titleEl.getBoundingClientRect().left / 2.5 / scale;
    //  console.log(translateX, translateY);

    let animation = anime
      .timeline({
        targets: this.polymorph,
        d: [
          {
            value:
              'M720, 0 H0 V 300 c 144, 0, 216 -72, 360 -72 s 216, 72, 360, 72, 216 -72, 360 -72, 216, 72, 360, 72V0Z',
          },
          {
            value:
              'M720, 0 H0 V 300 c 144 0, 216 0, 360 0 s 216 0, 360 0, 216 0, 360 0, 216 0, 360 0 V0Z',
          },
        ],
        easing: 'linear',
        loop: false,
        autoplay: false,
        duration: 500,
      })
      .add(this.getTranslateAnimation(this.getTranslation()), 0);

    resizeObserver.observe(this.container);

    const translateTitle = () => {
      console.log(window.pageYOffset / window.outerHeight);
      animation.seek((window.pageYOffset / window.outerHeight) * 1000);
    };

    fromEvent(document, 'scroll')
      .pipe(
        throttleTime(8),
        map((e) => {
          window.requestAnimationFrame(function () {
            translateTitle();
          });
        })
      )
      .subscribe();
    let observer = translateSubject.asObservable().pipe(
      debounceTime(10),
      distinctUntilChanged(
        (entry: ResizeObserverEntry, old: ResizeObserverEntry) =>
          entry.contentRect.width < 600 === old.contentRect.width < 600
      ),
      map((entry) => {
        const nextAnimation = this.getTranslateAnimation(this.getTranslation());
        animation.add(nextAnimation, 0);
        console.log(nextAnimation);
        translateTitle();
      })
    );
    observer.subscribe();
  }
  getVh(v) {
    var h = Math.max(
      document.documentElement.clientHeight,
      window.innerHeight || 0
    );
    return (v * h) / 100;
  }
  render() {
    return html`
      <!--Top blue portion-->
      <header class="header__grid">
        <div class="header__upper">
          <div class="header__title-wrapper">
            <div class="header__title__icon-wrapper">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 53.6535 43.2863"
                class="header__title__icon"
              >
                <defs>
                  <style>
                    .standard {
                      fill: #a7aef0;
                    }
                  </style>
                </defs>
                <g>
                  <g
                    style="transform-origin: center"
                    class="header__title__icon__path-groups"
                    id="Layer_2-2"
                    data-name="Layer 2"
                  >
                    <path
                      class="standard"
                      d="M20,21A21.7881,21.7881,0,0,1,0,43V37A15.8721,15.8721,0,0,0,13.2932,21,15.8551,15.8551,0,0,0,0,6V0A21.7733,21.7733,0,0,1,20,21Z"
                    />
                    <path
                      class="standard"
                      d="M22,0V6h7V37H22v6l14.3391.1755V.0718Z"
                    />
                    <path
                      class="standard"
                      d="M39.3145.1108v6h7v31h-7v6l14.339.1755V.1826Z"
                    />
                  </g>
                </g>
              </svg>
            </div>

            <div class="header__title__text">
              <slot name="header-title"></slot>
              <slot name="header-subtitle"></slot>
            </div>
          </div>
          <nav class="header__navigation">
            ${['about', 'portfolio', 'resume', 'interests'].map(
              (link) =>
                html`<a class="header__navigation__link" href="#${link}"
                  >${link}</a
                >`
            )}
          </nav>
        </div>
        <div class="header__toolbar">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            xmlns:xlink="http://www.w3.org/1999/xlink"
            viewBox="0 0 1440 330"
            preserveAspectRatio="none"
            class="header__toolbar__svg"
          >
            <defs>
              <style>
                .cls-1 {
                  fill: var(--background-color);
                  filter: url(#shadow);
                }
              </style>
              <filter id="shadow" x="0" y="0" width="200%" height="200%">
                <feDropShadow
                  dx="5"
                  dy="5"
                  stdDeviation="5"
                  flood-color="#8b8b8b"
                  flood-opacity="1"
                />
              </filter>
              <linearGradient
                id="linear-gradient"
                x1="720"
                y1="576"
                x2="720"
                gradientUnits="userSpaceOnUse"
              >
                <stop class="gradient__stop-1" offset="0.08" />
                <stop class="gradient__stop-2" offset="0.39" />
                <stop class="gradient__stop-3" offset="0.6" />
                <stop class="gradient__stop-4" offset="0.78" />
                <stop class="gradient__stop-5" offset="0.93" />
              </linearGradient>
            </defs>
            <g id="Layer_2" data-name="Layer 2">
              <g id="Layer_1-2" data-name="Layer 1">
                <!--If you are reading this: this was a pain to figure out (esp for someone who doesn't usually work with SVG). After the c character in the path, the formatting is as follows: x1 y1, x2 y2, x y. The c stands for 'curve'... I think. In this equation x y is the point, and x1y1 / x2y2 is the line representing the slope of the point. I wish we could just specify the slope, but alas not yet. David 05/29/2021 -->
                <path
                  class="cls-1 polymorph"
                  d="M720,0H0V300c144,0,216-72,360-72s216,72,360,72,216-72,360-72,216,72,360,72V0Z"
                />
              </g>
            </g>
          </svg>
        </div>
      </header>
    `;
  }

  static get styles() {
    return [style];
  }
}
