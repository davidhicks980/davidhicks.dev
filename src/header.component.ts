...
export class FoliageHeader extends LitElement {
  ...
  private duplicateNav() {
    const navNode = this.navEl.assignedNodes()[0] as HTMLElement;
    this.lowerNavNode = navNode.cloneNode(true);
    //  (this.navEl as HTMLElement).setAttribute('direction', 'column');
    this.upperNav.appendChild(this.lowerNavNode);
    (this.lowerNavNode as HTMLElement).firstElementChild.classList.add(
      'is-columnar'
    );
  }
  ...
  nav = html`<slot
    class="header__toolbar__content__nav"
    ...
    return html`
      ...
      <header class="header__grid">
        <div id="animeHeaderUpper" class="header__upper">
          ...
          <div class="header__upper__nav ${upperVisibility}"></div>
        </div>
