export const cqfill = (
  (
    ...
    supportsLayoutContainment = defaultRoot &&
      CSS.supports('contain: layout inline-size'),
    ...
    getSelectorText = (selectorList) =>
      selectorList.length
        ? `:where(${selectorList.join(',')})`
        : unmatchableSelector,
    ...
    hasInlineOuterDisplay = (element) =>
      /inline/i.test(getComputedStyle(element).display),

    /** @type {(cssParentRule: CSSParentRule, cssRule: CSSAnyRule) => number} */
    getCSSRuleIndexOf = (cssParentRule, cssRule) =>
      indexOf.call(cssParentRule.cssRules || [], cssRule),
    ...
    insertCssRule = (cssParentRule, cssText, index) =>
      cssParentRule.cssRules[cssParentRule.insertRule(cssText, index)],

    onResize = () => {
      for (const [
        containedSelectorText,
        innerRule,
        doesFulfillQuery,
        axis,
      ] of containerQueries) {
        ...
        for (const layoutContainer of layoutContainerSet) {
          if (
            doesFulfillQuery(
              layoutContainer,
              layoutContainerMap.get(layoutContainer)
            )
          ) {
            for (const element of layoutContainer.querySelectorAll(
              containedSelectorText
            )) {
        ...
        const nextSelectorText = fulfilledSelectorList.length
          ? `:is(${containedSelectorText}):where(${fulfilledSelectorList.join(
              ','
            )})`
          : unmatchableSelector;
    ...
    addLayoutContainerByCssRule = (
      ...
      const getFallbackCssText = (
        ...
        `${unmatchableSelector}{transform:scale3d(1,1,1);${
          hasInlineSizeContainment
            ? `inline-size:${hasInlineDisplay ? 0 : 100}%;`
            : ''
        }${
          hasBlockSizeContainment
            ? `block-size:${hasInlineDisplay ? 0 : 100};`
            : ''
        }}`;
      const fallbackCssText = `@media all{${getFallbackCssText(
        true
      )}${getFallbackCssText(false)}}`;
      const cssPolyfillGroup = insertCssRule(
        cssParentRule,
        fallbackCssText,
        cssRuleIndex
      );
      const [cssInlinePolyfillStyleRule, cssBlockPolyfillStyleRule] =
        cssPolyfillGroup.cssRules;
      ...
      const onMutation = () => {
        ...
        const doesMatchElement = (element, index) =>
          element === lastElements[index];

        const doesMatchAllElements =
          elements.length === lastElements.length &&
          every.call(elements, doesMatchElement);
        ...
        if (!doesMatchAllElements) {
          ...
          for (const element of elements) {
            ...
            layoutContainerMap.set(element, [
              hasInlineSizeContainment,
              hasBlockSizeContainment,
            ]);
            ...
            if (hasInlineOuterDisplay(element))
              inlineSelectorList.push(selectorText);
            else blockSelectorList.push(selectorText);

            ro.observe(element);
          }

          const nextInlinePolyfillStyleRuleSelectorText =
            getSelectorText(inlineSelectorList);
          if (
            cssInlinePolyfillStyleRule.selectorText !==
            nextInlinePolyfillStyleRuleSelectorText
          ) {
            cssInlinePolyfillStyleRule.selectorText =
              nextInlinePolyfillStyleRuleSelectorText;
          }

          const nextBlockPolyfillStyleRuleSelectorText =
            getSelectorText(blockSelectorList);
          if (
            cssBlockPolyfillStyleRule.selectorText !==
            nextBlockPolyfillStyleRuleSelectorText
          ) {
            cssBlockPolyfillStyleRule.selectorText =
              nextBlockPolyfillStyleRuleSelectorText;
          }
    ...
    polyfillLayoutContainment = (root, styleSheet) => {
      ...
      const getCssStyleRuleContainValues = (cssRule) =>
        cssRule.style
          ? cssRule.style
              .getPropertyValue('--css-contain')
              .trim()
              .toLowerCase()
              .split(/\s+/)
          : [];
      ...
      const walkCssParent = (cssParentRule) => {
        ...
        for (const cssRule of getCSSRules(cssParentRule)) {
          ...
          const hasInlineSizeContainment =
            hasLayoutContainment &&
            (hasSizeContainment || containValues.includes('inline-size'));
          const hasBlockSizeContainment =
            hasLayoutContainment &&
            (hasSizeContainment || containValues.includes('block-size'));
    ...
    polyfillContainerQueries = (root, styleSheet) => {
      ...
      const walkCssParent = (cssParentRule) => {
        ...
        for (const cssRule of getCSSRules(cssParentRule)) {
          ...
          const hasContainerQueryPolyfill =
            mediaText.indexOf('--css-container') === 0;

          if (hasContainerQueryPolyfill) {
            /** @type {null | [string, 'max' | 'min', 'height' | 'width', `${number}${string}`]} */
            const containerQueryMatches = cssRule.media[0].match(
              containerQueryMatcher
            );
            ...
            if (containerQueryMatches) {
              ...
              const doesFulfillQuery = (
                element,
                hasInlineSizeContainment,
                hasBlockSizeContainment
              ) => {
                const fulfillsBlockSizeContainment =
                  hasBlockSizeContainment !==
                  (axis === 'block-size' || axis === 'height');
                const fulfillsInlineSizeContainment =
                  hasInlineSizeContainment !==
                  (axis === 'inline-size' || axis === 'width');
                if (
                  !fulfillsBlockSizeContainment &&
                  !fulfillsInlineSizeContainment
                )
                  return false;
                ...
                const sized =
                  ...
                  (sizeUnit === 'em'
                    ...
                    : sizeUnit === 'rem'
                    ? parseInt(
                        window.getComputedStyle(root.documentElement).fontSize
                      )
                    : sizeUnit === 'vh'
                ...
                return minMax === 'min' ? value >= sized : value <= sized;
              };
              ...
              for (const cssInnerRule of getCSSRules(cssRule)) {
                ...
                if (cssInnerRuleSelectorText) {
                  const cssInnerRuleBlock = cssInnerRule.cssText.slice(
                    cssInnerRuleSelectorText.length
                  );
                  const cssPolyfillInnerRuleCssText = `${unmatchableSelector}${cssInnerRuleBlock}`;

                  /** @type {CSSStyleRule} */
                  const cssPolyfillInnerRule = insertCssRule(
                    cssPolyfillGroup,
                    cssPolyfillInnerRuleCssText,
                    index++
                  );
